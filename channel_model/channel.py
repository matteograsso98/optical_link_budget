...
class Channel:
    ...
    def __init__(self, channel_model_config) -> None:

        # self.endpoints = endpoints
        # self.geometry = geometry

        # Configuration parameters
        self.channel_scenario               = channel_model_config.get("channel_scenario")
        self.propagation_model              = channel_model_config.get("propagation_model")
        self.force_nlos_for_all_users       = channel_model_config.get("force_nlos_for_all_users")
        self.atmospheric_temperature_K      = channel_model_config.get("atmospheric_temperature_K")     # TODO Constant ?
        self.atmospheric_pressure_hPa       = channel_model_config.get("atmospheric_pressure_hPa")      # TODO Constant ?
        self.atmospheric_vapour_g_m3        = channel_model_config.get("atmospheric_vapour_g_m3")       # TODO Constant ?
        self.p                              = channel_model_config.get("p")


    def compute(self, ts_index : int, tx: Antenna, rx: Antenna, user_terminals_geodetic, slant_range, uv, thetaphi, elevation, user_indices=None):
        ...
        # lambda_m = tx.tx_element_spacing_lambda_factor
        ...
        phase_path_loss = (np.exp(-1j * slant_range * 2 * np.pi / lambda_m) / slant_range)
        ...
        if self._att_lut_dB is not None and user_indices is not None:
            # Fast path: read total atmospheric attenuation from the per-UT LUT.
            Att_dB_arr = self._lookup_attenuation_dB(np.asarray(user_indices), np.asarray(elevation))
            Att = Att_dB_arr * itur.u.dB
        else:
            f = tx.tx_carrier_frequency_hz * itur.u.Hz
            f = f.to(itur.u.GHz)
            D = rx.rx_antenna_diameter_m * itur.u.m
            p = self.p
        ...
        lsm = load_large_scale_model(self.channel_scenario)

        elev_clipped = np.clip(elevation, 10, 90)
        row_index = (np.round(elev_clipped, -1) / 10 - 1).astype(int)
        
        # Line-of-Sight probability
        los_prob = lsm["large_scale_fading"]['los_prob'].values[row_index]

        num, _ = user_terminals_geodetic.shape
        
        los_flag = np.random.rand(num) * 100 <= los_prob if self.propagation_model == "nlos" else np.ones(num, dtype=bool)

        sigma = np.where(
                los_flag,
                lsm["large_scale_fading"]['sigma_los'].values[row_index],
                lsm["large_scale_fading"]['sigma_nlos'].values[row_index]
            )
        clutter_loss = lsm["large_scale_fading"]['cl'].values[row_index]

        large_scale_loss_dB = np.random.randn(row_index.size) * sigma

        if self.propagation_model == "nlos":
            if self.force_nlos_for_all_users:
                large_scale_loss_dB += clutter_loss
            else:
                large_scale_loss_dB[~los_flag] += clutter_loss[~los_flag]
        
        large_scale_loss_dB = large_scale_loss_dB * itur.u.dB

        # print(f"Large Scale Model loss = {large_scale_loss_dB} {large_scale_loss_dB.shape}")

        total_loss_dB = large_scale_loss_dB + Att

        # print(f"Total Loss = {total_loss_dB} {total_loss_dB.shape}")
        
        total_loss_dB = total_loss_dB.value
        
        loss_scaling = 1.0 / np.sqrt(10 ** (0.1 * total_loss_dB))
        loss_scaling = loss_scaling[:, np.newaxis]  # Shape becomes (5915, 1)

        # print(f"Loss scaling = {loss_scaling} {loss_scaling.shape}")

        # print(f"Ideal Channel = {h_ideal} {h_ideal.shape}")

        h_realistic = h_ideal * loss_scaling
        # print(f"Realistic Channel = {h_realistic} {h_realistic.shape}")

        return h_ideal, h_realistic
        

    def _calculate_ideal_channel(self, slant_range: np.ndarray, radiation_pattern: np.ndarray, rx_gain_dB: float, noise_W: float, lambda_m: float) -> np.ndarray:
        #Calculates the ideal, lossless complex channel matrix based on geometry.
        # Normalization factor including receiver gain and free space path loss constant
        xi = (np.sqrt(10**(0.1 * rx_gain_dB) / noise_W) / ((4 * np.pi / lambda_m)))
    ...
    #     Initializes the Channel class with the simulation configuration.
    #     Expected to contain 'online', 'offline', and any 'derived' runtime tables.

    #     self.config = config
    #     self.channel_model_cfg = config.get('online', {}).get('channel_model', {})
    #     self.payload_cfg = config.get('online', {}).get('satellite_payload', {})

    # @staticmethod
    # def get_frequency_band(carrier_hz: float) -> str:
    ...
    # def user_link(self, users: dict, rp: np.ndarray) -> dict:

    #     Computes the channel specifically for the user link.
    #     Automatically grabs `user_terminal` definitions from the config.
    #     terminal_cfg = self.config.get('online', {}).get('user_terminal', {})
    #     return self._initialize_channel(terminal_cfg, users, rp)

    # def feeder_link(self, ground_stations: dict, rp: np.ndarray) -> dict:
    #     Computes the channel specifically for the feeder link.
    #     Automatically grabs `ground_station_terminal` definitions from the config.
    #     terminal_cfg = self.config.get('online', {}).get('ground_station_terminal', {})
    #     return self._initialize_channel(terminal_cfg, ground_stations, rp)

    def _initialize_channel(self, terminal_cfg: dict, nodes: dict, rp: np.ndarray) -> dict:
        #Core logic to initialize the channel for the first time (t=0).
        #Determines initial statistical profile (LOS/NLOS) and calculates initial matrices.
        # Read from the newly mapped YAML configuration paths
        propag_model = self.channel_model_cfg.get('propagation_model', 'los').lower()

        # --- 1. Calculate the ideal channel (H_LB) using the helper ---
        h_lb = self._calculate_ideal_channel(terminal_cfg, nodes['slant_range'], rp)
        ...
        if propag_model in ['los', 'nlos']:
            elev_clipped = np.clip(nodes['theta'], 10, 90)
            row_index = (np.round(elev_clipped, -1) / 10 - 1).astype(int)

            if propag_model == 'los':
                los_flag = np.ones_like(nodes['theta'], dtype=bool)
            else:
                # Assumes 'large_scale_fading' is pre-calculated and injected into config['derived']
                large_scale_df = self.config.get('derived', {}).get('large_scale_fading')
                los_prob = large_scale_df['los_prob'].values[row_index]
                los_flag = np.random.rand(nodes['theta'].size) * 100 <= los_prob

            # --- 3. Apply propagation losses using the helper ---
            h_tx = self._apply_propagation_losses(terminal_cfg, h_lb, nodes, row_index, los_flag)
    ...
    def _calculate_ideal_channel(self, terminal_cfg: dict, slant_range: np.ndarray, radiation_pattern: np.ndarray, rx_gain_dB: float, noise_W: float, lambda_m: float) -> np.ndarray:
        #Calculates the ideal, lossless complex channel matrix based on geometry.
    ...
    def _apply_propagation_losses(self, terminal_cfg: dict, ideal_channel: np.ndarray, nodes: dict, row_index: np.ndarray, los_flag: np.ndarray) -> np.ndarray:
        #Applies losses to the ideal channel, preventing double counting of atmospheric attenuation.
        propag_model = self.channel_model_cfg.get('propagation_model', 'los').lower()
        large_scale_df = self.config.get('derived', {}).get('large_scale_fading')
        ...
        if large_scale_df is not None:
            ...
            # Fallback if derived structures are missing to prevent execution crash
            sigma = np.zeros(row_index.size)
            clutter_loss = np.zeros(row_index.size)

        large_scale_loss_dB = np.random.randn(row_index.size) * sigma

        # Clutter only on NLOS (unless "force_nlos_for_all_users")
        if propag_model == 'nlos':
            force_nlos = self.channel_model_cfg.get('force_nlos_for_all_users', False)
            if force_nlos:
                large_scale_loss_dB += clutter_loss
            else:
                large_scale_loss_dB[~los_flag] += clutter_loss[~los_flag]
        ...
        # ---------------------------------------------------------
        el = np.asarray(nodes["theta"], dtype=float)
        el = np.clip(el, 0.1, 90.0)

        f_ghz = self.payload_cfg.get('carrier_frequency_hz', 20e9) * 1e-9

        # Pulling from new YAML locations
        p_exceed_percent = float(self.channel_model_cfg.get('p', 0.01))
        D_m = float(terminal_cfg.get('antenna_diameter_m', 0.6))

        # Receiver station height above mean sea level (km).
        if 'alt_km' in nodes:
            hs_km = np.asarray(nodes['alt_km'], dtype=float)
        else:
            # Assuming 0 if nodes dictionary doesn't specify altitude
            hs_km = 0.0 

        # Per-user lat/lon (deg) for P.618
        lat = np.asarray(nodes.get("lat", np.zeros_like(el)), dtype=float)
        lon = np.asarray(nodes.get("lon", np.zeros_like(el)), dtype=float)
